...
"""
Cython engine for collecting the full v12 distribution per radial bin.
"""
...
__all__ = ('v12_distribution_vs_r_engine', )

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.nonecheck(False)
def v12_distribution_vs_r_engine(double_mesh, x1in, y1in, z1in, x2in, y2in, z2in,
    vx1in, vy1in, vz1in, vx2in, vy2in, vz2in,
    squared_normalize_rbins_by_in, rbins_normalized, cell1_tuple):
    """
    Returns a list of lists: vrad_lists[i] contains all v12 values for annular bin i,
    where bin i covers rbins_normalized[i] < r <= rbins_normalized[i+1].
    No np.diff needed — pairs are assigned directly to their annular bin.
    """
    ...
    cdef int num_annular_bins = num_rbins_normalized - 1

    # One Python list per annular bin to collect vrad values
    vrad_lists = [[] for _ in range(num_annular_bins)]
                            ...
                            for i in range(0, Ni):
                                ...
                                for j in range(0, Nj):
                                    ...
                                    if drsq == 0:
                                        ...
                                        # Check if pair is within the outermost bin edge
                                        k = num_rbins_normalized - 1
                                        if normed_drsq <= rbins_normalized_squared[k]:
                                            vrad = (dx*dvx + dy*dvy + dz*dvz) / c_sqrt(drsq)

                                            # Walk down to find the annular bin:
                                            # k ends at the first edge index where
                                            # normed_drsq > rbins_sq[k-1], i.e. the pair
                                            # is in annular bin (k-1).
                                            while k > 0 and normed_drsq <= rbins_normalized_squared[k-1]:
                                                k -= 1

                                            # k==0 means d <= rbins[0] (inside innermost edge) → skip
                                            if k > 0:
                                                vrad_lists[k-1].append(vrad)
